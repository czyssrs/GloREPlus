    ...
    def __init__(self,
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("encoder"):
                ...
                self.enc = tf.nn.embedding_lookup(self.initial_embedding, self.x)
                
                ## Positional Encoding
                if hp.sinusoid:
                    self.enc += positional_encoding(self.x,
                                      num_units=hp.input_units, 
                                      zero_pad=False, 
                                      scale=False,
                                      scope="enc_pe")
                else:
            ...
            ### decoder: we don't have decoder here ###


            mid_layer = tf.reshape(self.enc, [-1, hp.maxlen * hp.hidden_units])

            ## scale to 1
            self.out_layer = tf.tanh(tf.layers.dense(mid_layer, hp.out_units))
...
def multihead_attention(queries, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # deal with dimension issue: input 300 hidden 512
        #outputs += Q
...
def label_smoothing(inputs, epsilon=0.1):
    ...
    ### as_list not work on unknown shape
    K = hp.num_class
